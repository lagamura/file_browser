...
public class FileBrowser extends JFrame {
  ...
  public void createTree() {
    ...
    TreeSelectionListener tListener =
  ...
  public void createSplitPane() {
    ...
    JSplitPane splitPane =
        new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, true, leftScroll, rightScroll);
  ...
  public void createLabel(File tempFile) {
    ...
    FileLabel label =
        new FileLabel("<html>" + tempFile.getName() + "</html>", icon, JLabel.CENTER, tempFile);
      ...
      if (searchClicked) {
        ...
        for (int j = i; j < dirNames.length; j++) {
          for (int k = 0; k < activeNode.getChildCount(); k++) {
            if (((FileNode) ((MyTreeNode) activeNode.getChildAt(k)).getUserObject())
                .getFileName()
                .equals(dirNames[j])) {
        ...
        for (int i = 0; i < activeNode.getChildCount(); i++) {
          if (((FileNode) ((MyTreeNode) activeNode.getChildAt(i)).getUserObject())
              .getFileName()
              .equals(file.getName())) {
        ...
        String input =
            (String)
                JOptionPane.showInputDialog(
                    null,
                    "Enter the name of the new Text Document:",
                    "Create Text",
        ...
        String input =
            (String)
                JOptionPane.showInputDialog(
                    ...
                    null,
                    null,
                    lastSingleClicked.getFile().getName());
        if (input != null) {
          MyTreeNode node = (MyTreeNode) tree.getLastSelectedPathComponent();
          ...
          for (int i = 0; i < node.getChildCount(); i++) {
            if (((FileNode) ((MyTreeNode) node.getChildAt(i)).getUserObject())
                .getFileName()
                .equals(oldFile.getName())) {
              tempName = oldFile.getName(); // Keep the old name in case of error
              tempIndex = i;
          ...
          if (oldFile.renameTo(newFile)) {
            ...
            JOptionPane.showMessageDialog(
                null, "Rename unsuccessful.", "Rename", JOptionPane.ERROR_MESSAGE);
            ((FileNode) ((MyTreeNode) node.getChildAt(tempIndex)).getUserObject())
                .setFileName(tempName); // in case of error restore the old file details
          }
        }
